...
class Decoder_proj(nn.Module):
    ...
    def forward(self, feature):
        ...
        dp0_out_seg = self.out_conv(x_d0)
        return dp0_out_seg, x_d0, x_d1, x_d2, x_d3
...
class UNet_mem(nn.Module):
    ...
    def forward(self, x, return_feature = False):
        feature = self.encoder(x)
        if reture_feature:
            output, feature = self.decoder(feature, return_feature = True)
...
class UNet_CCT(nn.Module):
    ...
    def forward(self, x, mutli = False):
        feature = self.encoder(x)
...
class UNet_CCT_EMB(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_CCT_EMB, self).__init__()
        ...
        params = {'in_chns': in_chns,
                  ...
                  'acti_func': 'relu'}
        self.encoder = Encoder(params)
        self.main_decoder = Decoder_proj(params)
        self.aux_decoder1 = Decoder_proj(params)
        
        self.contrast_embed_index = 0
        encoder_channels = params['feature_chns']
        contrast_embed_dim=128
        contrast_embed = True
        self.embed_head = EmbeddingHead(dim_in=encoder_channels[int(self.contrast_embed_index)],
    ...
    def forward(self, x, mutli = False,emb = False):
        feature = self.encoder(x)
        ...
        if mutli== True:
            if emb==True:
                #print('**********************')
                aux_seg1 = self.aux_decoder1(aux1_feature)
                feature_end = self.embed_head(aux_seg1[self.contrast_embed_index+1])
                feature_main = self.embed_head(main_seg[self.contrast_embed_index+1])
                return main_seg[0], aux_seg1[0], feature_main, feature_end
            
            else:
                #print('---------------------')
                aux_seg1 = self.aux_decoder1(aux1_feature)
                return main_seg[0], aux_seg1[0]
        else:
            return main_seg[0]
